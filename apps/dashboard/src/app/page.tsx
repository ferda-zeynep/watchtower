...
export default function DashboardPage() {
  ...
  useEffect(() => {
    ...
    fetch(`${apiUrl}/api/events`, { cache: "no-store" })
      .then((res) => {
        if (!res.ok) throw new Error("Failed to fetch data from API server.");
        return res.json();
      })
      .then((resData) => {
    ...
    socket.on("new-event", (newEvent: TelemetryEvent) => {
      setEvents((prevEvents) => [newEvent, ...prevEvents]);
      setToastMessage(
        `🚨 New ${newEvent.type} received from ${newEvent.projectKey}!`,
      );
  ...
  return (
    ...
    >
      ...
      <div style={{ marginBottom: "2rem" }}>
        <button
          onClick={() => {
            throw new Error(`Real-Time Update Test #${Date.now()}`);
          }}
          style={{
